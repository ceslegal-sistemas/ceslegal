...
Cada proceso recibe un codigo unico con formato `PD-YYYY-XXXX`, donde `YYYY` es el anio y `XXXX` un consecutivo de cuatro digitos. Este codigo se genera automaticamente al crear el proceso y sirve como identificador en todos los documentos y comunicaciones.
...
apertura --> descargos_pendientes --> descargos_realizados --> sancion_emitida --> cerrado
                                  --> descargos_no_realizados --> sancion_emitida --> cerrado
                                                                                  --> archivado
```
...
| `descargos_realizados` | Trabajador completo la diligencia | Verde |
| `descargos_no_realizados` | Trabajador no asistio | Naranja |
| `sancion_emitida` | Sancion emitida y notificada | Rojo |
| `impugnacion_realizada` | Trabajador impugno la sancion | Morado |
| `cerrado` | Proceso finalizado | Gris |
| `archivado` | Proceso archivado sin sancion | Gris claro |

Desde cualquier estado activo se puede transicionar a `archivado`.

### Asignacion de Abogado
Al crear o editar un proceso, se asigna un abogado (`abogado_id`) que sera responsable del caso. El abogado recibe una notificacion automatica de prioridad alta al ser asignado.
...
Al hacer clic en la accion "Enviar Citacion", el sistema:
...
- **Al enviar la citacion**: Genera preguntas iniciales estandar (10) + preguntas especificas basadas en los hechos (hasta 5) + preguntas de cierre (3).
- **Durante la diligencia**: Genera preguntas dinamicas basadas en las respuestas del trabajador (hasta 2 por respuesta, maximo 30 preguntas totales).
...
- **Enviar Citacion**: Disponible en estado `apertura`.
- **Emitir Sancion**: Disponible en estados `descargos_realizados` o `descargos_no_realizados`.
- **Archivar**: Disponible en cualquier estado activo.
- **Cerrar**: Disponible en estado `sancion_emitida`.
- **Ver Timeline**: Disponible siempre.
