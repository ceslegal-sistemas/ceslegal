            ...
            Alpine.data('completarIA', function (init) {
                return {
                    ...
                    init() {
                        ...
                        // Filtrar cuando cambia el texto (cliente)
                        this.$wire.$watch('data.descripcion_hecho', () => this.sync());

                        // Actualizar cuando el servidor cambia sugerencias
                        this.$wire.$watch('sugerenciasCompletado', (val) => {
                            this.all = val || [];
                            this.sync();
                        });
                    },
                    ...
                    sync() {
                        const texto = this.$wire.get('data.descripcion_hecho') || '';
                        this.visible = (this.all || []).filter(s => texto.includes(s.marker));
                        if (this.idx >= this.visible.length) this.idx = 0;
                    },

                    get current() { return this.visible[this.idx] ?? null; },
                    ...
                    next() { if (this.idx < this.visible.length - 1) { this.idx++; this.mostrarPersonalizado = false; this.valorPersonalizado = ''; } },
                    prev() { if (this.idx > 0) { this.idx--; this.mostrarPersonalizado = false; this.valorPersonalizado = ''; } },
                };
            });
        });
    }
        ...
        <div class="phc-body"
            ...
            <div class="phc-chips">
                ...
                <button type="button" class="phc-btn-otra"
                    ...
                    @click="mostrarPersonalizado = true; $nextTick(() => $refs.customInput?.focus())">
            ...
            <div class="phc-custom-row"
                ...
                <input type="text"
                    ...
                    @keydown.enter.prevent="if (valorPersonalizado.trim()) aplicar(current.marker, valorPersonalizado.trim())"
                    @keydown.escape="mostrarPersonalizado = false; valorPersonalizado = ''">
                <button type="button" class="phc-btn-aplicar"
                    @click="if (valorPersonalizado.trim()) aplicar(current.marker, valorPersonalizado.trim())">
                    <svg style="width:10px;height:10px" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
